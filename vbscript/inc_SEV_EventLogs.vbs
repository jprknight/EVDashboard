...
If LoggingLevel >= 1 then
	wscript.echo DateTime() & ": Processing Symantec Enterprise Vault Event Logs."
	If LoggingLevel >= 2 then
		OutputLogFile.writeline ""
		OutputLogFile.writeline "*************************************************************************"
		OutputLogFile.writeline ""
	End if
	OutputLogFile.writeline DateTime() & ": Processing Symantec Enterprise Vault Event Logs."
End if
...
For i = 0 to Ubound(EVServersList,2)
	...
	'* MINIMAL LOGGING.
	If LoggingLevel >= 1 then
		wscript.echo DateTime() & ": (SEV Event Logs) " & EVServersList(1,i)
		OutputLogFile.writeline DateTime() & ": (SEV Event Logs) " & EVServersList(1,i)
	End if
	
	Set objWMI = GetObject("winmgmts:{impersonationLevel=impersonate}!\\" & EVServersList(1,i) & "\root\cimv2")
	Set colLoggedEvents = objWMI.ExecQuery ("Select * from Win32_NTLogEvent Where Logfile = 'Symantec Enterprise Vault' " & _
	...
	For Each objEvent in colLoggedEvents
		If objEvent.SourceName = "Enterprise Vault" Then
			...
			If DateDiff("d",strTimeWritten,now) < 1 then
				...
				'* NORMAL LOGGING.
				If LoggingLevel >= 2 then
					'wscript.echo DateTime() & ": (SEV Event Logs) : Event Code - " & objEvent.EventCode  & " - Event Type: " & objEvent.Type
					OutputLogFile.writeline DateTime() & ": (SEV Event Logs) : Event Code - " & objEvent.EventCode  & " - Event Type: " & objEvent.Type
				End if
				...
				If LoggingLevel >= 3 then
					...
					OutputLogFile.writeline DateTime() & ": (SEV Event Logs) - strSQLQuery1" & vbcrlf & strSQLQuery1
				End if
