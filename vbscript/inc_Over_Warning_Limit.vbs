...
If LoggingLevel >= 1 then
	wscript.echo DateTime() & ": Processing Over Warning Limit."
	OutputLogFile.writeline ""
	OutputLogFile.writeline "*************************************************************************"
	OutputLogFile.writeline ""
	OutputLogFile.writeline DateTime() & ": Processing Over Warning Limit."
End if
...
If LoggingLevel >= 2 then
	'wscript.echo DateTime() & ": (Over Warning Limit) - Connection1" & vbcrlf & Connection1
	OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - Connection1" & vbcrlf & Connection1
End if
...
strSQLQuery1 = "Use EnterpriseVaultDirectory " & _
	...
	"MBxSize/1000 - MbxWarningLimit/1000 as 'Over Warning (MB)', " & _
	...
	"WHERE " & _
	"EME.MBxWarningLimit > 0 AND " & _
	"EME.MbxArchivingState = 1 AND " & _
	"EME.MBxSize > EME.MbxWarningLimit and " & _
...
If LoggingLevel >= 3 then
	'wscript.echo DateTime() & ": (Over Warning Limit) - strSQLQuery1" & vbcrlf & strSQLQuery1
	OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - strSQLQuery1" & vbcrlf & strSQLQuery1
End if		
...
While not result1.EOF
	...
	'* Manipulate data to be inserted into EVDashboard database.
		...
		If InStr(strMailbox,chr(39)) then
			...
			'* VERBOSE LOGGING.
			If LoggingLevel >= 3 then
				'wscript.echo DateTime() & ": (Over Warning Limit) - Apostrophe Fix" & vbcrlf & strMailbox
				OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - Apostrophe Fix" & vbcrlf & strMailbox
			End if
		end if
		' apostrophe fix end.
		...
		'* VERBOSE LOGGING.
		If LoggingLevel >= 3 then
			'wscript.echo DateTime() & ": (Over Warning Limit) - ExchangeState - " & ExchangeState
			OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - ExchangeState - " & ExchangeState
		End if
		
	RecordCount = RecordCount + 1
	'* MINIMAL LOGGING.
	If LoggingLevel >= 1 then
		wscript.echo DateTime() & ": (Over Warning Limit) - " & RecordCount & " - " & strMailbox
		OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - " & RecordCount & " - " & strMailbox
	End if
	...
	'* Connect to EVDashboard database and insert data.
		...
		'* NORMAL LOGGING.
		If LoggingLevel >= 2 then
			'wscript.echo DateTime() & ": (Over Warning Limit) - Connection2" & vbcrlf & Connection2
			OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - Connection2" & vbcrlf & Connection2
		End if
		...
		strSQLQuery2 = "UPDATE dbo.OverWarningLimit " & _
			"SET Mailbox='" & strMailbox & "'," & _
			"OverWarningMB='" & (result1("Over Warning (MB)")) & "'," & _
			...
			"IF @@ROWCOUNT=0 " & _
			"INSERT INTO dbo.OverWarningLimit " & _
			"(Mailbox,OverWarningMB,WarningMB,SendMB,ReceiveMB,MailboxSizeMB,NumItemsMailbox," & _
			"ExchangeServer,LastArchived,ExchangeState,PolicyUsed) " & _
			"VALUES ('" & strMailbox & "'," & (result1("Over Warning (MB)")) & "," & (result1("Warning (MB)")) & _
		...
		'* VERBOSE LOGGING.
		If LoggingLevel >= 3 then
			'wscript.echo DateTime() & ": (Over Warning Limit) - strSQLQuery2" & vbcrlf & strSQLQuery2
			OutputLogFile.writeline DateTime() & ": (Over Warning Limit) - strSQLQuery2" & vbcrlf & strSQLQuery2
		End if
