...
If LoggingLevel >= 1 then
	wscript.echo DateTime() & ": Processing Over Receive Limit."
	OutputLogFile.writeline ""
	OutputLogFile.writeline "*************************************************************************"
	OutputLogFile.writeline ""
	OutputLogFile.writeline DateTime() & ": Processing Over Receive Limit."
End if
...
If LoggingLevel >= 2 then
	'wscript.echo DateTime() & ": (Over Receive Limit) - Connection1" & vbcrlf & Connection1
	OutputLogFile.writeline DateTime() & ": (Over Receive Limit) - Connection1" & vbcrlf & Connection1
End if
...
If EVVersion = 1 then
	...
	strSQLQuery1 = "Use EnterpriseVaultDirectory " & _
		...
		"MBxSize/1000 - MbxReceiveLimit/1000 as 'Over Receive (MB)', " & _
		...
		"WHERE " & _
		"EME.MBxReceiveLimit > 0 AND " & _
		"EME.MbxArchivingState = 1 AND " & _
		"EME.MBxSize > EME.MbxReceiveLimit and " & _
...
If LoggingLevel >= 3 then
	'wscript.echo DateTime() & ": (Over Receive Limit) - strSQLQuery1" & vbcrlf & strSQLQuery1
	OutputLogFile.writeline DateTime() & ": (Over Receive Limit) - strSQLQuery1" & vbcrlf & strSQLQuery1
End if	
...
While not result1.EOF
	...
	'* Connect to EVDashboard database and insert data.
		...
		strSQLQuery2 = "UPDATE dbo.OverReceiveLimit " & _
			"SET Mailbox='" & strMailbox & "'," & _
			"OverReceiveMB='" & (result1("Over Receive (MB)")) & "'," & _
			...
			"IF @@ROWCOUNT=0 " & _
			"INSERT INTO dbo.OverReceiveLimit " & _
			"(Mailbox,OverReceiveMB,WarningMB,SendMB,ReceiveMB,MailboxSizeMB,NumItemsMailbox," & _
			"ExchangeServer,LastArchived,ExchangeState,PolicyUsed) " & _
			"VALUES ('" & strMailbox & "'," & (result1("Over Receive (MB)")) & "," & (result1("Warning (MB)")) & _
