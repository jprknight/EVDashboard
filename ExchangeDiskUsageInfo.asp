...
%>
<!-- Main Form -->
<form id="frmconfig" method="post" action="ExchangeDiskUsageInfo.asp">
<!-- Table for page heading -->
<table width="100%" class="Main">
<tr>
<td nowrap="nowrap" width="450px">
<h1>Exchange Disk Usage Information</h1>
</td>
...
Function HistoryData
	...
	SQLQuery = ("Select EVServer,EVRole,Drive,VolumeName,SizeGB,UsedGB," & _
		"FreeGB,PercentageFree from DiskUsageInfo " & SQLWhereClause & " AND EVRole = 'Email'")
...
Function LiveData
	...
	'* Execute the query
	Set result = EVDmyconn.execute("select ExchangeServer " & _
	...
	While not result.EOF
		...
		CurrentEXServer = (result("ExchangeServer"))
		...
		For Each objItem in colItems
			...
			If pctFreeSpace < 25 then
				Response.Write("<tr><td><h5>" & UCase(CurrentEXServer) & "</h5></td>" & _
					"<td><h5>" & objItem.Name & "</h5></td><td><h5>" & objItem.VolumeName & _
					"</h5></td>" & "<td><h5>" & strDiskSize & "</h5></td><td><h5>" & _
					strUsedSpace & "</h5></td><td><h5>" & strFreeSpace & "</h5></td><td><h5>" & _
					pctFreeSpace & "</h5></td></tr>")
			Else
				Response.Write("<tr><td>" & UCase(CurrentEXServer) & "</td><td>" & objItem.Name & "</td><td>" & _
					objItem.VolumeName & "</td><td>" & strDiskSize & "</td><td>" & strUsedSpace & _
					"</td><td>" & strFreeSpace & "</td><td>" & pctFreeSpace & "</td></tr>")
			End if
		Next
		result.movenext()
	Wend
